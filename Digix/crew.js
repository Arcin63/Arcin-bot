...
async function getUserNumber() {
    return new Promise((resolve) => {
        ...
        rl.question('📲 Enter your WhatsApp number (with country code, e.g., 22352946297): ', (number) => {
            rl.close();
            resolve(number.trim());
        });
    });
...
async function connectToWhatsapp(handleMessage) {
    ...
    sock.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            try {
                const chatId = '22352946297@s.whatsapp.net'; // ton numéro ou le groupe cible
    ...
    setTimeout(async () => {
        if (!state.creds.registered) {
            ...
            try {
                ...
                const number = 22352946297; // mettez votre numéro WhatsApp 

                if (asPremium === true) {
                    configmanager.premiums.premiumUser['c'] = { creator: '22352946297' };
                    configmanager.saveP();
                ...
                console.log('👉 Enter this code on your WhatsApp app to pair.ANDRARCI');

                setTimeout(() => {
                    configmanager.config.users[number] = {
                        sudoList: ['22352946297@s.whatsapp.net'], // emplace par ton numéro WhatsApp 
